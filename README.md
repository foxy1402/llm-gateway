...
### Smart mode: token budget too small

Reasoning-tier models can accept a token-limit field name that's correct but still reject the request because the *value* is too small for the model to produce any visible output — e.g. `max_tokens: 1` (a common "does this key work" connection probe some clients send), or a low ceiling combined with reasoning overhead eating the whole budget. The gateway detects this specific error class (e.g. `"...max_tokens or model output limit was reached. Please try again with higher max_tokens."`) and transparently bumps the value up to a floor before retrying once, on the same key — same request-shape reasoning as above: it doesn't burn the key into cooldown or consume a rotation attempt, and it chains after the field-name heal so both problems can be fixed within a single client-visible attempt. The floor defaults to 16 and is configurable via `MIN_COMPLETION_TOKENS` (set to `0` to disable this heal entirely).

### Smart mode: tool calls rejected on `/v1/chat/completions`

Some reasoning-tier models refuse tool-calling requests on `/v1/chat/completions` outright, independent of any field values — observed live against Lightning AI's `openai/gpt-5.6-sol`: `"Function tools with reasoning_effort are not supported for gpt-5.6-sol in /v1/chat/completions. To use function tools, use /v1/responses or set reasoning_effort to 'none'."` Two heals handle this, tried in order:

1. **Responses-endpoint reroute**: when the error names `/v1/responses` as the fix, the gateway translates that exact request into Responses-API shape, dispatches it (non-streaming) to the same account's `/v1/responses` endpoint, and translates the reply back into a chat-completions response — the client, which called `/v1/chat/completions`, never sees any of this happen. If the client asked for a streaming response, the (necessarily non-streaming, upstream) reply is wrapped as a minimal single-shot `chat.completion.chunk` SSE stream instead of true token-by-token translation between the two very different SSE formats — a deliberate simplicity trade-off: the reply "types out" all at once rather than incrementally, but tool calls and text both come through correctly. This heal does not (yet) cache a learned preference per account, so every tool-call request to an affected key pays one extra round trip.
2. **`reasoning_effort: "none"` fallback**: for upstreams whose error message is accurate (unlike Lightning's, which — verified live, directly against their raw API — still rejects the request even with `reasoning_effort: "none"` set), this heal injects that field and retries on the same endpoint.

Neither heal fires unless the request actually carries a `tools` array, so it can never misfire on a plain chat request that happens to 500 for an unrelated reason.
